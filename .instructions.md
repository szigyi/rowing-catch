...
plot_transforms/ ← imports from (algo, scenario)
    ↑
alo/, scenario/  ← imports from each other only
```
...
- **Type Checking**: `python -m mypy rowing_catch/ --ignore-missing-imports`
...
```bash
# Type checking
python -m mypy rowing_catch/ --ignore-missing-imports
