...
Renders SPM vs drive/recovery ratio scatter plot with ideal ratio curve and cycle labels.
...
from rowing_catch.plots.theme import BG_COLOR_AXES, COLOR_IDEAL_RATIO, COLOR_MAIN, COLOR_TEXT_SUB
from rowing_catch.plots.utils import setup_premium_plot


def render_rhythm_consistency(computed_data: dict[str, Any]) -> None:
    """Render rhythm consistency scatter plot with ideal ratio curve.
    ...
    ratio_vals = np.array(data['ratio_vals'])
    ...
    if not np.isnan(mean_ratio):
        ax.axhline(mean_ratio, color='#94a3b8', linewidth=1, linestyle='--', alpha=0.7, label='Mean D:R')
