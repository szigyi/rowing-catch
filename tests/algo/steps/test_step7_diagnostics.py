...
from rowing_catch.algo.steps.step7_diagnostics import _detect_outliers_zscore, step7_diagnostics
...
def test_step7_diagnostics_basic():
    ...
    avg_cycle = pd.DataFrame({'Seat_X_Smooth': np.linspace(0, 100, 10)})
    ...
    assert diag['sampling_is_stable']
...
def test_step7_diagnostics_unstable_sampling():
    ...
    assert not diag['sampling_is_stable']
...
def test_detect_outliers_zscore():
    ...
    assert outliers_robust[-1], 'Extreme outlier should be detected'
