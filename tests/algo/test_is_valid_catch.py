...
def test_is_valid_catch_basic():
    ...
    assert _is_valid_catch(x, 5, min_separation=2, prominence=None, snr_threshold=0)
...
def test_is_valid_catch_not_minimum():
    ...
    assert not _is_valid_catch(x, 1, min_separation=2, prominence=None, snr_threshold=0)
...
def test_is_valid_catch_shallow_dip():
    ...
    assert not _is_valid_catch(x, 50, min_separation=10, prominence=None, snr_threshold=0)
...
def test_is_valid_catch_too_high():
    ...
    assert _is_valid_catch(x, 2, min_separation=1, prominence=None, snr_threshold=0)
    ...
    assert _is_valid_catch(x2, 2, min_separation=1, prominence=None, snr_threshold=0)
    ...
    assert _is_valid_catch(x3, 2, min_separation=1, prominence=None, snr_threshold=0)
    
    # To fail: x[idx] > mean_val
    # x = [0, 10, 9, 10, 0]. Mean = 5.8. x[2]=9. 9 > 5.8 -> FAIL.
    # But x[2]=9 is a local minimum? Yes.
    x4 = np.array([0, 10, 9, 10, 0])
    assert not _is_valid_catch(x4, 2, min_separation=1, prominence=None, snr_threshold=0)
...
def test_is_valid_catch_prominence():
    ...
    assert not _is_valid_catch(x, 1, min_separation=5, prominence=3.0, snr_threshold=0)
...
def test_is_valid_catch_secondary_signal():
    ...
    assert _is_valid_catch(x, 2, min_separation=2, prominence=None, secondary_signal=y, snr_threshold=0)
...
def test_is_valid_catch_rejected_by_secondary_signal():
    ...
    assert not _is_valid_catch(x, 2, min_separation=2, prominence=None, secondary_signal=y_bad, snr_threshold=1000)
...
def test_is_valid_catch_boundary_leeway():
    ...
    assert _is_valid_catch(x, 2, min_separation=10, prominence=None, snr_threshold=0)
